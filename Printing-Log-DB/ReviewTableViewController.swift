...
class ReviewTableViewController: UITableViewController {
    ...
    func updateUserInterface(){
        ...
        if review.documentID == "" { // If true, this is a new review
           addBordersToEditableObjects()
        } else {
            if review.reviewUserID == Auth.auth().currentUser?.uid { // Review is posted by current user
                self.navigationItem.leftItemsSupplementBackButton = false
                saveBarButton.title = "Update"
                addBordersToEditableObjects()
                deleteButton.isHidden = false
            } else { // Review is posted by different user
                saveBarButton.hide()
    ...
    @IBAction func reviewTitleChanged(_ sender: UITextField) {
        ...
        let noSpacews = reviewTitleField.text!.trimmingCharacters(in: .whitespacesAndNewlines)
        if noSpacews != "" {
    ...
    @IBAction func saveButtonPressed(_ sender: UIBarButtonItem) {
        ...
        review.saveData(spot: spot) { (success) in
            if success {
                ...
                print("ERROR: Can't unwind from segue from Review because of saving error...")
            }
        }
